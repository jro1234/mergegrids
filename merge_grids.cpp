...
//  printf("\nn entries    : %d\n", target_block->n_entries);
  ...
  //printf("\n n_exclude    = %d", n_exclude);
  //printf("\n n_kk_exclude = %d", n_kk_exclude);
  //printf("\n datablock->xm= %d", data_block->xm);

  //printf("\nDoing values from %d to %d", n_exclude, target_block->n_entries + n_exclude);
  ...
  for (int i=n_exclude; i<data_block->n_entries - n_exclude; i++) {
    if (i%data_block->xm >= n_kk_exclude && i%data_block->xm < data_block->xm-n_kk_exclude) {
     ...
     //   printf("\nwriting from block %d:  i: %d   i_new: %d", data_block->kk, i, i_entry);
    if (data_block->data[i] > 1e-10) {
     ...
     //   printf("   new   value : block %d, grid %d:  %d  %.9f\n", target_block->kk, target_block->id, i_entry, target_block->data[i_entry]);

     //   float old_value = target_block->data[i_entry] - reduce_factor * data_block->data[i];
     //   if (old_value > 1e-10){
     //     printf("   old   value : block %d, grid %d:  %d  %.9f\n", target_block->kk, target_block->id, i_entry, old_value);
     //   }
     // }
    }
  }

        //printf("\nMEMORY SHOULD BE FREER NOW!");
...
void Reduce_UHBD_Grids (UHBD_Grid* target_grid, UHBD_Grid* data_grid, int resize_grid) {
  ...
  if (resize_grid > 0) {
    ...
    // note that for a cubic grid (the only kind this code handles currently)
    // n_kk_exlude is also equal to the n_elements_excluded when viewing each
    // block as a matrix. (ie the distance inwards from matrix edges)
  ...
  printf("\nReducing blocks now");
  printf("\nn_kk_exclude:        %d", n_kk_exclude);
  printf("\ntarget_grid->factor: %f", target_grid->factor);
  printf("\nkk_max:              %d", kk_max);
  ...
  for (int kk=n_kk_exclude; kk<kk_max; kk++) {
    ...
    ///////////////////////////////////////
    ///////////////////////////////////////
      //printf("\n Doing block #%d (old block #%d)", new_kk, kk);
      target_grid->blocks[new_kk]->xm = target_grid->header.xm;

      // this all happens only once per block
      // after the first block is reduced, the block->factor is no longer zero
      if (target_grid->blocks[new_kk]->factor < 1e-10) {
        ...
        //printf("\nPrinting the raw block header now...");
        //Print_UHBD_Block_Header(target_grid->blocks[new_kk]->raw_header);
      ...
      //printf("\ntarget grid block factor is: %f", target_grid->blocks[new_kk]->factor);
      //printf("\ntarget grid block  kk    is: %d", target_grid->blocks[new_kk]->kk);
...
void Merge_UHBD_Grid (std::vector<UHBD_Grid*> vec_grids, int* parts_target_grid, int n_thread, int n_threads) {
  ...
  for (int i=blocks_interval[0]; i<=blocks_interval[1]; i++) {
    //printf("\nIn thread %d, block %d", n_thread, i);
...
//void Compare_UHBD_Grids(UHBD_Grid* grid1, UHBD_Grid* grid2, int resize) {
//
...
//    int kk_shifted;
//
//    kk_shifted = kk + resize / 2;
//
//    block1 = grid1->blocks[kk];
//    block2 = grid2->blocks[kk_shifted]
//
//    for (int i=i_min; i<i_max; i++){
//      float difference;
//
//      if () {
//        difference = block1[i] - block2[i_shifted];
//        if (difference > 1e-4) {
//        }
//      }
//    }
...
void Resize_UHBD_Grid (std::string p_grid, int resize_grid) {

        FILE* f_data_grid;
        if ((f_data_grid = fopen(p_grid.c_str(), "rb")) == NULL) {
          printf("Unable to open file: %s\n", p_grid.c_str());
        }

        UHBD_Grid* data_grid = Read_UHBD_Header(f_data_grid);
                   data_grid->factor = 1;
                   data_grid->id = 0;

        Read_UHBD_Grid(data_grid, f_data_grid);

        UHBD_Grid* target_grid;
                   target_grid = Initialize_Target_Grid(data_grid, resize_grid);

        Reduce_UHBD_Grids(target_grid, data_grid, resize_grid);

        delete data_grid;

  std::string p_target_grid;
  FILE* f_target_grid;

  if (resize_grid > 0) {
    p_target_grid = p_grid + "." + std::to_string(resize_grid);
    if ((f_target_grid = fopen(p_target_grid.c_str(), "wb")) == NULL) {
      printf("Unable to open file: %s\n", p_target_grid.c_str());
      exit(0);
    }
  }

  Write_UHBD_Grid(f_target_grid, target_grid);
...
void Merge_UHBD_Grids (int** lists_p_grids, std::vector<std::string>& vec_p_grids, std::string d_run_home, int n_threads, int n_grids_thread, int n_grids, int resize_grid) {
  ...
  #pragma omp parallel
  {
    ...
    for (int i=0; i<n_grids_thread; i++) {
     ...
     // }
     // else { usleep(999999999); }

//        if (i==0) {
//            Compare_UHBD_Grids(vec_merged_grids[n_thread], data_grid, resize_grid);
//        }
...
int main(int argc, char ** argv)
{
  ...
  if (argc==3) {
    ...
    std::string p_grid;
    p_grid = d_run_home + argv[3];
    if (file_exists(p_grid)) {
      vec_p_grids.push_back(p_grid);
      n_grids = 1;
    } else {
      printf("\nCannot locate grid file: %s", p_grid.c_str());
      return 1;
    }
    resize_grid = atoi(argv[4]);
  } else {
    printf("\nCannot interpret arguments\n");
      return 1;
  }

  if (n_grids == 1) {
    printf("\nResizing grid to a %d-sided cube\n\n", resize_grid);
    Resize_UHBD_Grid(vec_p_grids[0], resize_grid);
    printf("\n");
