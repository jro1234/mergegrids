    ...
    for ii in range(1,1+len(grid_list)):
        ...
        while not ready:
            ...
            if not f_block_state:
                print "Validating input files ready for merging"
                f_block_state = f_block_state_fresh
            elif len(f_block_state) == len(f_block) and set(f_block_state) == set(f_block_state_fresh):
                print "Input files ready for merger, block {0} of {1}".format(ii,len(grid_list))
                ready = True
            else:
